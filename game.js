...
function startBusiness(i){
  ...
  b.running=true;b.progress=0;renderBusinesses();
}
      ...
      <div class="card-main">
        ...
        <div class="buy-row">
          ...
          <button class="buy-btn yellow" data-run="${i}" ${b.running||b.count===0?"disabled":""}>
            <span class="price">${fmt(payout(i))}</span><span class="buy-meta">${b.manager?"auto running":"run cycle"}</span>
          </button>
        </div>
        <div class="sub-row">
...
function loop(now){
  ...
  renderHeader();
  document.querySelectorAll("[data-progress]").forEach(el=>{const i=+el.dataset.progress;el.style.width=(state.businesses[i].progress*100)+"%"});
